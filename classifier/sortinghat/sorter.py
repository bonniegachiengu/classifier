...
class SortingHat:
    ...
    def analyzeStructure(self, genres, studios):
        ...
        for parent, child in filtered_dag.items():
            if self.isMovie(parent, child):
                ...
                print(f"Classified {parent} as Movie")  # Debug statement
    ...
    def isMovie(self, parent, children):
        ...
        print(f"Checking if {parent} is a movie: {bool(match)}")  # Debug statement
        return bool(match)
    ...
    def saveResults(self):
        ...
        with sqlite3.connect(self.dbpath) as conn:
            cursor = conn.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS classifications (folder TEXT PRIMARY KEY, classification TEXT)")
